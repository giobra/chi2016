...
goal, and analyse the consequences on the reliability of the resulting
system. Compared to MIGTool, that approach cannot take advantage of a
model of the system and has instead to rely only on models of the
...
time to decide what goal to achieve, to understand how to do it, to
locate the actions to do it, to perform the action, to attend to the
feedback. Layout of the user interface, Fitt's law considerations,
cognitive state of the user, physical and environmental aspects affect
such times. For these reasons we opted, in \migtool, to only focus on
required steps to complete a task, not on completion time.
...
gets an estimation of how long an experienced user would spend on each
...
completion times that are cognitively justified. However, with our
method, you can analyze a large part of the UI, get information about
...
CogTool. Approaches like \cite{swearngin13} start from the assumption
...
24 states, 12 pseudo states, 12 regions, and 61 transitions. The model
is similar to that being discussed in examples mentioned by \cite{ifml13}.
...
sequences. We believe this in practice greatly reduces usefulness of
...
when it is disengaged it remembers the currently set speed, which can be
...
\item system S features a more compact design, with fewer action types
...
of states, or with distributing differently concurrent regions across
...
broadcasting mechanism) upon which data binding is applied, so that
transitions can be easily bound to data instances. On the other hand,
designers using \migtool\ do not need to cope with data modeling, nor
with decisions dealing with presentation. In a sense, \migtool\ uses
only the \emph{controller} part of the Model-View-Controller paradigm
that is adopted when developing UIs. Although in principle the same
results could be obtained from IFML models, to do so one needs to
isolate the control part of the IFML model from the data part; this is
difficult to do because, by design, IFML integrates very well the two
aspects.
...
\item enables grounding of tasks/scenarios through a
  ...
  \emph{abstract-user-interface} level (we are referring to the
...
compatibility of the system with respect to tasks. With an extended
version of \migtool\ it could be done without having to implement the
system. 
...
underway; it will be freely available for research purposes. Currenlty
