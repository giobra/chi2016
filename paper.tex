...
supported.
%
Adopting Usage Centered Development (UCD) practices is effective, as
...
models can be used to specify alternative ways to achieve te same
goal, and analyse the consequences on safety and reliability of the resulting
system. Compared to MIGTool, this approach cannot take advantage of a
model of the system and  instead relies only on models of the
tasks. The conseguence is that analysing alternative ways to achieve
the same goal requires developing different models; in addition,
exploring suitability of a given system with respect to certain tasks
is complex; and finally, hierarchical task models cannot take
advantage of contexts and rich control flows that are available with
state-transition representations, limiting therefore the richness of
the scenarios being analysed.
...
execution traces.  They are potential paths (\ie, sequences of
...
graph. In general a specification of a scenario includes an initial state and a
...
can be be isolated and/or ignored~\cite{gb15:icwe}.
...
as specification of the scenarios to be analyzed. At that point,
\migtool\ could be used as a tool to explore suitability of tasks with
respect to a behavioral model of a user interface. This problem has
been addressed in \cite{martinie15} through a mechanism that relates
task models to source code annotations of the implementation of the
user interface. In this way it is possible to simulate execution of
the tasks in parallel with execution of the system and ensure
...
underway; it will be freely available for research purposes. Currently
